      ...
      <div className="max-w-[210mm] mx-auto bg-white text-black min-h-[297mm] shadow-lg p-8 sm:p-12 print:shadow-none print:p-0 print:m-0">
        ...
        {hasNoData ? (
          <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-gray-400">
            ...
            <p className="text-sm mt-2">Load data or start filling in sections from the sidebar</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <header id="section-personal-info" className="flex flex-col items-center justify-center border-b-2 border-black pb-6 mb-6">
              {fullName && (
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 mt-4 text-sm font-medium text-gray-600">
                {personalInfo?.phone && (
                  <span className="inline-flex items-center gap-1">
                    <Image src={CallIcon} alt="Phone" width={14} height={14} />
                    {personalInfo.phone}
                  </span>
                )}
                {personalInfo?.email && (
                  <span className="inline-flex items-center gap-1">
                    <Image src={MailIcon} alt="Email" width={14} height={14} />
                    {personalInfo.email}
                  </span>
                )}
              ...
              {socialMedia.length > 0 && (
                <div id="section-social-media" className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 mt-2 text-sm text-gray-600">
                  {socialMedia.map((sm) => (
                    <span key={sm.id}>{sm.platform}: {sm.username || sm.url}</span>
                  ))}
                </div>
              )}
            </header>
              ...
              <div className="md:col-span-1 flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-4">
                      {education.map((edu) => (
                        <div key={edu.id}>
                          ...
                          <p className="text-sm italic text-gray-700">{edu.degree}{edu.field ? ` in ${edu.field}` : ""}</p>
                ...
                {technicalSkills.length > 0 && (
                  <section id="section-technical-skills">
                    <h2 className="text-lg font-bold uppercase border-b-2 border-black pb-1 mb-3">Technical Skills</h2>
                ...
                {softSkills.length > 0 && (
                  <section id="section-soft-skills">
                    <h2 className="text-lg font-bold uppercase border-b-2 border-black pb-1 mb-3">Soft Skills</h2>
                    <p className="text-sm leading-relaxed text-gray-800">{softSkills.join(", ")}</p>
                  </section>
                )}
                ...
                {additionalSkills.length > 0 && (
                  <section id="section-additional-skills">
                    <h2 className="text-lg font-bold uppercase border-b-2 border-black pb-1 mb-3">Additional Skills</h2>
                    <p className="text-sm leading-relaxed text-gray-800">{additionalSkills.join(", ")}</p>
                  </section>
                )}
                ...
                {languages.length > 0 && (
                  <section id="section-languages">
                    <h2 className="text-lg font-bold uppercase border-b-2 border-black pb-1 mb-3">Languages</h2>
                    <div className="flex flex-col gap-1">
                      {languages.map((lang) => (
                        <p key={lang.id} className="text-sm text-gray-800">
              ...
              <div className="md:col-span-2 flex flex-col gap-6">
                ...
                {workExperience.length > 0 && (
                  <section id="section-work-experience">
                    <h2 className="text-lg font-bold uppercase border-b-2 border-black pb-1 mb-4">Work Experience</h2>
